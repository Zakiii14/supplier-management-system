...
const envArguments = rawArguments.filter((value) =>
...
dotenv.config(
  envPath ? { path: path.resolve(envPath) } : undefined,
);
...
const argumentsWithoutEnv = rawArguments.filter(
  (value) => !value.startsWith("--env="),
);
const action = argumentsWithoutEnv.shift();
if (!ACTIONS.has(action)) {
  throw new Error(
    "Usage: backup.js <create|verify|restore> [backup-directory] [options]",
  );
}

const flags = new Set(
  argumentsWithoutEnv.filter((value) => value.startsWith("--")),
);
const positional = argumentsWithoutEnv.filter(
...
for (const flag of flags) {
  if (!supportedFlags.has(flag)) {
    throw new Error(`Unknown backup option: ${flag}`);
  }
}
...
const getBackupRoot = () => {
  ...
  if (process.env.NODE_ENV === "production" && !configured) {
    throw new Error(
      "BACKUP_ROOT is required for production backup operations",
    );
  }
  if (
...
const assertFilesystemStorageTargets = (storageTargets) => {
  const unsupported = storageTargets.filter(
    (target) => target.provider !== "filesystem" || !target.directory,
  );
  if (unsupported.length > 0) {
    throw new Error(
      "Backup create/restore currently supports FILE_STORAGE_PROVIDER=filesystem only. Blob-backed demo deployments use the dedicated demo reset workflow.",
    );
  }
};
...
const main = async () => {
  ...
  if (action === "create") {
    if (
      positional.length > 0 ||
      flags.has("--confirm-restore")
    ) {
      throw new Error(
        "backup:create does not accept a backup directory or --confirm-restore",
      );
    }
    requireMaintenanceConfirmation();
    assertFilesystemStorageTargets(storageTargets);
    ...
    const result = await createBackupBundle({
      ...
      consistencyMode: flags.has("--maintenance")
        ? "maintenance"
        : "live",
    });
  ...
  if (positional.length !== 1) {
    throw new Error(
      `${action} requires exactly one backup directory path`,
    );
  }
  ...
  if (action === "verify") {
    if (flags.size > 0) {
      throw new Error("backup:verify does not accept flags");
    }
  ...
  console.log(
    `Restore completed from backup created at ${manifest.created_at}.`,
  );
  console.log(
    "Run npm run migrate:status before starting application traffic.",
  );
};
