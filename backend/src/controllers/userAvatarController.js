...
const { isDemoMode } = require("../middleware/demoModeMiddleware");
const {
  storeAvatar,
  removeAvatar,
  streamAvatar,
...
const validateAccess = (
  ...
  const canManageOtherUser =
    isAdmin && (allowAdminInDemo || !isDemoMode());

  if (!canManageOtherUser && req.user.id !== req.params.id) {
    res.status(403).json({
      success: false,
...
const getUserAvatar = async (req, res) => {
  if (!validateAccess(req, res, { allowAdminInDemo: true })) return;
...
const replaceUserAvatar = async (req, res) => {
  ...
  try {
    ...
    const current = await pool.query(
      `SELECT avatar_storage_name FROM app.users WHERE id = $1`,
      [req.params.id],
    );
    ...
    await pool.query(
      ...
      UPDATE app.users
      SET
        avatar_storage_name = $1,
        avatar_original_name = $2,
        avatar_mime_type = $3,
        avatar_size_bytes = $4,
