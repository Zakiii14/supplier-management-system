...
const getAllPayments = async (req, res) => {
	try {
      ...
      `);
		...
		if (method) {
			values.push(method);
			conditions.push(`p.method::TEXT = $${values.length}`);
		}
		if (invoice_id) {
...
const createPayment = async (req, res) => {
	...
	try {
		...
		if (!payment_number || !invoice_id || amount === undefined || !method) {
			return res.status(400).json({
				success: false,
				message: "payment_number, invoice_id, amount, and method are required",
			});
		}
      ...
      `,
		...
		const outstandingAmount = Number(invoice.grand_total) - Number(invoice.paid_amount);
		if (Number(amount) > outstandingAmount) {
			throw new Error(`Payment exceeds outstanding amount (${outstandingAmount})`);
		}
      ...
      `,
		...
		if (dateResult.rows[0].invalid_date) {
			throw new Error("Payment date cannot be earlier than invoice date");
		}
