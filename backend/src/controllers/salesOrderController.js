...
const getAllSalesOrders = async (req, res) => {
	try {
		...
		const allowedStatuses = ["DRAFT", "CONFIRMED", "PARTIALLY_DELIVERED", "DELIVERED", "CANCELLED", ];
      ...
      `);
		...
		if (status) {
			values.push(status);
			conditions.push(`so.status::TEXT = $${values.length}`);
		}
		if (customer_id) {
			values.push(customer_id);
			conditions.push(`so.customer_id = $${values.length}`);
		}
