  ...
  try {
    ...
    if(decision==="APPROVED"){
      ...
      for(const item of items.rows){
        const stockBucket=item.item_condition==="SALEABLE"?"AVAILABLE":item.item_condition;
        await client.query(`INSERT INTO app.inventory_movements(product_id,movement_type,quantity,reference_type,reference_id,stock_bucket,notes,created_by) VALUES($1,'RETURN_IN',$2,'SALES_RETURN',$3,$4,$5,$6)`,[item.product_id,item.quantity,req.params.id,stockBucket,`Retur penjualan ${current.rows[0].return_number}`,req.user.id]);
        if(stockBucket==="AVAILABLE")await client.query(`UPDATE app.products SET current_stock=current_stock+$1,updated_at=NOW() WHERE id=$2`,[item.quantity,item.product_id]);
        else if(stockBucket==="QUARANTINE")await client.query(`UPDATE app.products SET quarantine_stock=quarantine_stock+$1,updated_at=NOW() WHERE id=$2`,[item.quantity,item.product_id]);
        else await client.query(`UPDATE app.products SET damaged_stock=damaged_stock+$1,updated_at=NOW() WHERE id=$2`,[item.quantity,item.product_id]);
      }
    }
    const result=await client.query(`UPDATE app.sales_returns SET status=$1,decided_by=$2,decided_at=NOW(),rejection_reason=$3 WHERE id=$4 RETURNING *`,[decision,req.user.id,decision==="REJECTED"?reason:null,req.params.id]);await client.query(`INSERT INTO app.transaction_approvals(transaction_type,transaction_id,action,reason,acted_by) VALUES('SALES_RETURN',$1,$2,$3,$4)`,[req.params.id,decision,decision==="REJECTED"?reason:null,req.user.id]);await client.query("COMMIT");return res.json({success:true,message:decision==="APPROVED"?"Retur disetujui; inventory diperbarui sesuai kondisi barang":"Retur penjualan ditolak",data:result.rows[0]});
