...
const getSupplierInvoice = async (req,res) => {
  ...
  const [attachments,payments,returnSettlements]=await Promise.all([
    ...
    pool.query(`SELECT prs.id,prs.settlement_type,prs.settlement_date,prs.amount,prs.reference_number,pr.return_number
      FROM app.purchase_return_settlements prs
      JOIN app.purchase_returns pr ON pr.id=prs.purchase_return_id
      JOIN app.goods_receipts gr ON gr.id=pr.goods_receipt_id
      WHERE prs.supplier_invoice_id=$1
         OR (prs.settlement_type='REFUND' AND gr.purchase_order_id=$2)
      ORDER BY prs.settlement_date DESC,prs.created_at DESC`,[req.params.id,result.rows[0].purchase_order_id]),
  ]);
  return res.json({success:true,data:{...result.rows[0],attachments:attachments.rows,payments:payments.rows,return_settlements:returnSettlements.rows}});
};
...
const saveSupplierInvoice = async (req,res) => {
  ...
  try {
    ...
    const po=await client.query(`SELECT po.id,po.status,COALESCE((SELECT SUM(poi.quantity*poi.unit_price) FROM app.purchase_order_items poi WHERE poi.purchase_order_id=po.id),0) total FROM app.purchase_orders po WHERE po.id=$1 FOR UPDATE OF po`,[poId]);
    ...
    if (total!==Number(po.rows[0].total)) { const e=new Error(`Total tagihan harus sama dengan nilai PO (${Number(po.rows[0].total)})`); e.statusCode=400; throw e; }
    const existingInvoice=await client.query(`SELECT id,invoice_number FROM app.supplier_invoices WHERE purchase_order_id=$1 AND ($2::uuid IS NULL OR id<>$2::uuid) LIMIT 1`,[poId,id||null]);
    if(existingInvoice.rows.length){const e=new Error(`Purchase order sudah memiliki tagihan supplier (${existingInvoice.rows[0].invoice_number})`);e.statusCode=409;throw e;}
    if(id){
      const usage=await client.query(`SELECT COALESCE((SELECT SUM(amount) FROM app.supplier_payments WHERE supplier_invoice_id=$1),0) paid,COALESCE((SELECT SUM(amount) FROM app.purchase_return_settlements WHERE supplier_invoice_id=$1 AND settlement_type IN ('INVOICE_DEDUCTION','SUPPLIER_CREDIT')),0) credited,COUNT(*)::integer attachments FROM app.supplier_invoice_attachments WHERE supplier_invoice_id=$1`,[id]);
      if(total<Number(usage.rows[0].paid)+Number(usage.rows[0].credited)){const e=new Error("Total tagihan tidak boleh lebih kecil dari pembayaran dan kredit retur yang sudah tercatat");e.statusCode=400;throw e;}
    ...
    await client.query(`UPDATE app.supplier_payments SET supplier_invoice_id=$1,supplier_invoice_number=$2 WHERE purchase_order_id=$3 AND (supplier_invoice_id IS NULL OR supplier_invoice_id=$1)`,[result.rows[0].id,result.rows[0].invoice_number,poId]);
